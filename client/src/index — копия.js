...
import { addMessage, addMessagesHistory, addUsersList, setUser } from "./actions/index";

const store = createStore(rootReducer);

//store.dispatch({type: 'SET_USER', uid: 123})
...
class LoginApp extends React.Component {
	...
	render() {
		console.log(store.getState().isUserLoggedIn);
		console.log(store.getState().isUserLoggedIn === false);
		return(
			<div>
				{store.getState().isUserLoggedIn === false ? <LoginWindow error={this.state.error} checkEnterKey={this.checkEnterKey} /> : ""}
			</div>
		)
	}

	setUser = (uid) =>{
		store.dispatch({type: 'SET_USER', uid: uid})
	}
	...
	componentWillMount() {
		...
		store.subscribe(() => {
			console.log(store.getState().isUserLoggedIn+' login');
		})
    }
}
...
class ChatContainer extends React.PureComponent { 
	...
	render() { 
		...
		<section id='chatSection' className="container">
			<ChatMessagesBlock uid={this.state.uid} usersList={this.state.usersList} messageEdited={this.messageEdited} />
	...
	componentWillMount() {
		socket.on('chatHistory', (data) => {
			...
			store.dispatch( addUsersList(data.userList));
		});
		socket.on('userLoggedIn', (data) => {
...
class ChatMessagesBlock extends React.Component { 
	state = {
		showStickers : false,
	}

	messageEdited = (mid = -1, message = '') => {
		this.props.messageEdited(mid, message);
	}
		...
		return (
			<div className="row">
				<div className={this.state.showStickers === false ? transition+'col-11' : transition+"col-8"}>
					<Chat uid={this.props.uid} usersList={this.props.usersList} editMessage={this.messageEdited} edditingMessage={this.props.midEdit} />
				</div>
...
class Chat extends React.Component { 
	...
	render() { 
		...
		return (
		    <div id="chat">
				{( this.state.chatHistory && Object.keys(this.props.usersList).length > 0) ? Object.keys(this.state.chatHistory).map(i => {
					...
					return [
						...
						(this.state.chatHistory[i]['isSystem'] === false && userChanged === true) ? 
							<UsernameItem 
								userName={this.props.usersList[this.lastMessageUid]['name']}  
								userColor={this.props.usersList[this.lastMessageUid]['color']} 
								date={this.state.chatHistory[i]['date']} 
								unread={this.state.chatHistory[i]['unread']} 
							/>: '',
						(this.state.chatHistory[i]['isSystem'] === true) ?
							<SystemMessage 
								message={this.state.chatHistory[i]['msg']} 
								isDate="false" 
							/> : 
							(this.state.chatHistory[i]['isSticker'] === false) ? <MessageItem 
									key={i}
									message={this.state.chatHistory[i]} 
									uid={this.props.uid} 
									editMessage={this.editMessage} 
									mid={i} 
									isEdditing={this.props.edditingMessage === i ? true : false} 
								/> :
								<div className={(this.state.chatHistory[i]['unread'] === true ? 'unreadMessages stickerMessage' : 'stickerMessage')}>
									<SingleSticker key={"sticker_"+this.state.chatHistory[i]['msg']} i={this.state.chatHistory[i]['msg']} />
								</div>
					]
...
class Stickers extends React.Component {
	render() { 
		var img = [];
		for(var i = 1; i <= 48; i++){ 
			img.push(<SingleSticker key={"sticker_"+i} click={this.sendSticker} i={i} />)
		}
		return (
...
class SingleSticker extends React.Component {
	render() { 
		return (
		    <img key={this.props.i+'_sticker'} alt="" src={stickerPath+this.props.i+".png"} className="sticker" onClick={() => this.props.click(this.props.i)} />
		)
	}
}
	...
	render() { 
		return (
		    <div className="form-group">
				<textarea className="form-control" rows="3" onKeyDown={(e) => this.keyPress(e)} ref="form-control" id="textfield" ></textarea>
			</div>
		);
	} 
